...
if ( ! class_exists( 'RWMB_BTText_Field' ) && class_exists( 'RWMB_Field' ) ) {
	class RWMB_BTText_Field extends RWMB_Field {
		...
		static function html( $meta, $field ) {	
			...
			return sprintf(
				'<input type="hidden" class="rwmb-text" name="%s" id="%s" value="%s" placeholder="%s" size="%s" %s>%s',
				$field['field_name'],
				$field['id'],
				$meta,
				$field['placeholder'],
	...
	function bt_dump_grid( $arr, $json ) {
		...
		foreach ( $arr as $post ) {
			...
			if ( $blog_author || $blog_date ) {
				...
				if ( $blog_author ) $meta .= __( 'by', 'bt_theme' ) . ' ' . $post['author'];
				$meta .= '</p>';
			}			
	...
	function bt_get_posts_data( $number, $offset, $cat_slug, $limit ) {
		...
		if ( $number > 0 ) {
			if ( $cat_slug != '' ) {
				...
				$cat_id = $cat->term_id;
				$recent_posts = wp_get_recent_posts( array( 'numberposts' => $number, 'post_status' => 'publish', 'offset' => $offset, 'category' => $cat_id ) );
			} else {
				$recent_posts = wp_get_recent_posts( array( 'numberposts' => $number, 'post_status' => 'publish', 'offset' => $offset ) );
			}
		} else {
			$recent_posts = array();
		}
		...
		foreach ( $recent_posts as $post ) {
			$posts_data[ $i ]['permalink'] = get_permalink( $post['ID'] );
			$posts_data[ $i ]['format'] = get_post_format( $post['ID'] );
			$posts_data[ $i ]['title'] = $post['post_title'];
			$posts_data[ $i ]['date'] = date_i18n( $date_format, strtotime( get_the_time( 'Y-m-d', $post['ID'] ) ) );
			...
			$posts_data[ $i ]['images'] = bt_rwmb_meta( BTPFX . '_images', 'type=image', $post['ID'] );
			if ( $posts_data[ $i ]['images'] == null ) $posts_data[ $i ]['images'] = array();
			$posts_data[ $i ]['video'] = bt_rwmb_meta( BTPFX . '_video', array(), $post['ID'] );
			$posts_data[ $i ]['audio'] = bt_rwmb_meta( BTPFX . '_audio', array(), $post['ID'] );
			$posts_data[ $i ]['grid_gallery'] = bt_rwmb_meta( BTPFX . '_grid_gallery', array(), $post['ID'] );
			$posts_data[ $i ]['link_title'] = bt_rwmb_meta( BTPFX . '_link_title', array(), $post['ID'] );
			$posts_data[ $i ]['link_url'] = bt_rwmb_meta( BTPFX . '_link_url', array(), $post['ID'] );
			$posts_data[ $i ]['quote'] = bt_rwmb_meta( BTPFX . '_quote', array(), $post['ID'] );
			$posts_data[ $i ]['quote_author'] = bt_rwmb_meta( BTPFX . '_quote_author', array(), $post['ID'] );
			$posts_data[ $i ]['ID'] = $post['ID'];
...
if ( ! function_exists( 'bt_inline_style' ) ) {
	function bt_inline_style() {
		...
		$body_font = esc_html( bt_get_option( 'body_font' ) );
		$heading_font = esc_html( bt_get_option( 'heading_font' ) );
...
if ( ! function_exists( 'bt_load_fonts' ) ) {
	function bt_load_fonts() {
		...
		if ( $body_font != 'no_change' || $heading_font != 'no_change' ) {
			wp_register_style( 'bt_fonts', esc_url_raw( 'http://fonts.googleapis.com/css?family=' . $fonts ) );
